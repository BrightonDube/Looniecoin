...
const char * const BITCOIN_CONF_FILENAME = "looniecoin.conf";
const char * const BITCOIN_PID_FILENAME = "looniecoind.pid";
...
#else
    const char* pszModule = "looniecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Looniecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Looniecoin";
#else
    // Unix
    return pathRet / ".looniecoin";
#endif
#endif
}
